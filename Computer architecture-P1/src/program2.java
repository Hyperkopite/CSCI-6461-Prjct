...
public class program2 {
	public static void main(String[] args) throws Exception {
		...
		String userInput = "123 "; // mimic user input
		...
		for (int i = ca.START; i < ca.STOP; i++) {
			...
			if (Arrays.toString(ca.r[1]).equals(Arrays.toString(ca.r[2]))) {
				wordNum++;
				compare = true;
				continue;
			} // word number +1 when meet character Space.
			if (Arrays.toString(ca.r[1]).equals(Arrays.toString(ca.r[3]))) {
				sentenceNum++;
				wordNum = 0;
			} // sentence number +1 when meet character period.
			if (compare) {
				ca.Ldr(Integer.toBinaryString(SEARCH + increment), 0, 0, ms); // store the first character of userInput
																				// to R0.
				...
				increment++;
				if (increment == userInput.length() - 1) {
					find = true;
					break;
				} // minus the last character, which is ETX.
			}
		}
